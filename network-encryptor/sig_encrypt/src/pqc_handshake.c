...
typedef struct {
    bool is_initiator;
    char peer_ip[64];
    char identity_priv[1024];
    char identity_pub[1024];
} hs_config_t;
...
static void* pqc_handshake_thread(void* arg) {
    ...
    if (g_hs_cfg.is_initiator) {
        ...
        calculate_auth_tag(g_hs_cfg.identity_priv, msg->data, pk_sz, msg->auth_tag);
        ...
        while (!g_key_ready) {
            ...
            if (n > 0) {
                ...
                if (resp->magic == PQC_HS_MAGIC && resp->msg_type == PQC_HS_MSG_RESP) {
                    ...
                    calculate_auth_tag(g_hs_cfg.identity_pub, resp->data, resp->data_len, expected_tag);
        ...
        while (!g_key_ready) {
            ...
            if (n > 0) {
                ...
                if (msg->magic == PQC_HS_MAGIC && msg->msg_type == PQC_HS_MSG_HELLO) {
                    // Verify Auth Tag (Using peer's pub identity)
                    uint8_t expected_tag[PQC_AUTH_TAG_SZ];
                    calculate_auth_tag(g_hs_cfg.identity_pub, msg->data, msg->data_len, expected_tag);
                    ...
                    if (trf_kem_encapsulate(msg->data, msg->data_len, ct, &ct_sz, ss) == TRF_PQC_OK) {
                        ...
                        // Calculate Auth Tag (Sign with our priv identity)
                        calculate_auth_tag(g_hs_cfg.identity_priv, resp->data, ct_sz, resp->auth_tag);
...
int sig_pqc_handshake_start(bool is_initiator, const char *peer_ip, 
    ...
    if (identity_priv) strncpy(g_hs_cfg.identity_priv, identity_priv, 1023);
    if (identity_pub)  strncpy(g_hs_cfg.identity_pub,  identity_pub,  1023);
